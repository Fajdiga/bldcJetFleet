...
#ifdef LSM6DS3_HW_I2C
static i2c_hw_state m_i2c_hw;
static bool m_use_hw_i2c = false;
#endif
...
// I2C transaction wrapper - uses hardware or bitbang I2C depending on configuration
static bool lsm6ds3_i2c_tx_rx(uint16_t addr, uint8_t *txbuf, size_t txbytes, uint8_t *rxbuf, size_t rxbytes) {
#ifdef LSM6DS3_HW_I2C
	if (m_use_hw_i2c) {
		return i2c_hw_tx_rx(&m_i2c_hw, addr, txbuf, txbytes, rxbuf, rxbytes);
	}
#endif
	return i2c_bb_tx_rx(m_i2c_bb, addr, txbuf, txbytes, rxbuf, rxbytes);
}
...
void lsm6ds3_init(i2c_bb_state *i2c_state,
	...
	bool res = lsm6ds3_i2c_tx_rx(lsm6ds3_addr, txb, 1, rxb, 1);
	if (!res || (rxb[0] != 0x69 && rxb[0] != 0x6A && rxb[0] != 0x6C)) {
		...
		res = lsm6ds3_i2c_tx_rx(lsm6ds3_addr, txb, 1, rxb, 1);
		if (!res || (rxb[0] != 0x69 && rxb[0] != 0x6A && rxb[0] != 0x6C)) {
	...
	res = lsm6ds3_i2c_tx_rx(lsm6ds3_addr, txb, 2, rxb, 1);
	if (!res){
	...
	lsm6ds3_i2c_tx_rx(lsm6ds3_addr, txb, 2, rxb, 1);
...
static uint8_t read_single_reg(uint8_t reg) {
	...
	txb[0] = reg;
	bool res = lsm6ds3_i2c_tx_rx(lsm6ds3_addr, txb, 1, rxb, 2);
...
static THD_FUNCTION(lsm6ds3_thread, arg) {
	...
	while (!chThdShouldTerminateX()) {
		...
		bool res = lsm6ds3_i2c_tx_rx(lsm6ds3_addr, txb, 1, rxb, 12);
...
void lsm6ds3_init_hw_i2c(I2CDriver *i2c_dev,
	...
	// Configure hardware I2C state
	m_i2c_hw.i2c_dev = i2c_dev;
	m_i2c_hw.sda_gpio = sda_gpio;
	m_i2c_hw.sda_pin = sda_pin;
	m_i2c_hw.scl_gpio = scl_gpio;
	m_i2c_hw.scl_pin = scl_pin;
	m_i2c_hw.gpio_af = gpio_af;
	m_i2c_hw.speed = speed;

	i2c_hw_init(&m_i2c_hw);
	...
	bool res = lsm6ds3_i2c_tx_rx(lsm6ds3_addr, txb, 1, rxb, 1);
	if (!res || (rxb[0] != 0x69 && rxb[0] != 0x6A && rxb[0] != 0x6C)) {
		commands_printf("LSM6DS3 HW I2C Address A failed, trying B (rx: %d)", rxb[0]);
		lsm6ds3_addr = LSM6DS3_ACC_GYRO_ADDR_B;
		res = lsm6ds3_i2c_tx_rx(lsm6ds3_addr, txb, 1, rxb, 1);
		if (!res || (rxb[0] != 0x69 && rxb[0] != 0x6A && rxb[0] != 0x6C)) {
			commands_printf("LSM6DS3 HW I2C Address B failed (rx: %d)", rxb[0]);
			return;
		}
	}
